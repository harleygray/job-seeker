...
defmodule SeekScraper do
  use Wallaby.DSL
  import Wallaby.Query, only: [css: 1, css: 2]
  alias JobHunt.{Repo, Job}

  def run do
    IO.puts("Starting SeekScraper...")
    ...
    try do
      ...
      session
      |> visit(
        "https://www.seek.com.au/Data-Analyst-jobs/in-Queensland-QLD?daterange=1&worktype=244%2C242"
      )
  ...
  defp extract_employer_text(element) do
    try do
      find(element, css("a"))
      |> Wallaby.Element.text()
    rescue
      Wallaby.QueryError ->
        # If we can't find an 'a' tag, just use the text of the span
  ...
  defp extract_job_details(session) do
    ...
    try do
      title =
        session
        |> find(css("h1[data-automation='job-detail-title']"))
        |> Wallaby.Element.text()
      ...
      employer =
        case Wallaby.Browser.find(session, css("span[data-automation='advertiser-name']")) do
          {:ok, span_element} ->
            extract_employer_text(span_element)
...
IO.puts("Starting SeekScraper.run()...")
SeekScraper.run()
IO.puts("SeekScraper.run() completed.")
