<script lang="ts">
    ...
    const filteredResumes = $derived(
        resumes.filter(resume => {
          ...
          const matchesType = filterType === 'all' || 
            (filterType === 'experience' && resume.experience?.length > 0) ||
    ...
    function getTypeVariant(type: string): "default" | "secondary" | "destructive" | "failed" | "pending" | "in_progress" | "complete" | "official" {
...
<div class="resume-container">
    ...
    <div class="resume-list flex-1 overflow-y-auto px-4 pt-2 pb-0" bind:this={listElement}>
        {#if loading}
                ...
                {#each filteredResumes as resume (resume.id)}
                    <li>
                        <button 
                            ...
                            class:border-primary-300={selectedResumeId === resume.id}
                            class:border-gray-200={selectedResumeId !== resume.id}
                            onclick={() => selectResume(resume.id)}
                        >
                            <div class="flex justify-between items-start">
                                <div class="flex-grow">
                                    <div class="font-medium text-gray-900">
                                        {resume.experience?.[0]?.title || 'Untitled Resume'}
                                    </div>
                                ...
                                <div class="flex gap-2">
                                    {#if resume.experience?.length}
                                        <Badge variant="complete" class="text-xs capitalize">
