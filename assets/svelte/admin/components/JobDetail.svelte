<script lang="ts">
    ...
    import { Root, Trigger, Content } from "$lib/components/ui/popover/index.js"; // Corrected import path
    ...
    $: {
        if (creatingJob) {
            ...
            formData = {
                ...
                software_engineer_score: selectedJob.software_engineer_score || 0,
            ...
            editMode = false;
            formData = { title: "", description: "", status: "", employer: "", location: "", salary: "", apply_link: "", seek_job_id: "", formatted_description: "", data_analyst_score: 0, data_engineer_score: 0, data_scientist_score: 0, software_engineer_score: 0, ai_engineer_score: 0, product_owner_score: 0, applied: false, archived: false };
        }
    }
    ...
    function updateJobStatus(newStatus: string) {
        if (!selectedJob || !live) {
            console.error("Cannot update status: No selected job or live connection.");
            statusPopoverOpen = false;
            return; 
        }

        console.log(`Updating status for job ${selectedJob.id} to: ${newStatus}`);
        ...
        live.pushEvent("update_job", { id: selectedJob.id, status: newStatus }, (reply) => {
    ...
    function saveJob() {
        ...
        if (live) {
            ...
            live.pushEvent(event, payload, (reply) => {
                if (reply && reply.success) {
                    console.log(`Job ${creatingJob ? 'created' : 'updated'} successfully:`, reply);
                    if (!creatingJob) {
                        editMode = false; // Exit edit mode after successful update
                    }
                    // Optionally reset creatingJob state via LiveView event if needed
                } else {
                    console.error(`Error ${creatingJob ? 'creating' : 'updating'} job:`, reply);
                }
            });
        }
    }
    ...
    onMount(() => {
        console.log("JobDetail mounted. Creating:", creatingJob, "Selected:", selectedJob);
...
<div class="p-4 space-y-4 h-full flex flex-col bg-white">
    ...
    {:else}
        ...
        <div class="flex justify-between items-center pb-2 border-b">
            ...
            <div class="flex items-center gap-3">
                 <h2 class="text-xl font-semibold">
                    {creatingJob ? 'Create New Job' : (selectedJob?.title || 'Job Details')}
                 </h2>
                 ...
                 {#if selectedJob || creatingJob} 
                     <Root bind:open={statusPopoverOpen}>
                        <Trigger class="text-left">
                            <button type="button" class="p-0">
                                <Badge 
                                    ...
                                    class="text-xs cursor-pointer hover:opacity-80 capitalize justify-start py-1 px-2 border {selectedJob ? 'border-gray-300' : 'border-dashed border-gray-400'}"
                                >
                                    {formData.status}
                                </Badge>
                            </button>
                        ...
                        {#if selectedJob} 
                            <Content class="z-50 w-[300px] rounded-md border bg-white p-4 shadow-md">
                                <div class="flex flex-col gap-2">
                                    <p class="text-sm font-medium mb-2">Update Status</p>
                                    <div class="grid grid-cols-2 gap-2">
                                        {#each statusOptions as status}
                                            <button 
                                            type="button"
                                            class="capitalize {status === formData.status ? 'bg-blue-600 text-white' : 'bg-gray-100 border border-gray-200'} p-2 rounded-md hover:bg-opacity-90 text-sm"
                                            on:click={() => updateJobStatus(status)} 
                                            >
        ...
        {/if}
                ...
                {:else}
                    <Badge variant="secondary" class="text-xs italic">No Job Selected</Badge>
                {/if}
                                    </div>
        ...
        <div class="flex-grow space-y-4 overflow-y-auto">
            <!-- Title and Employer Row -->
                    ...
                    {#if editMode}
                        ...
                        />
                                        {:else}
                        <div class="p-2 bg-gray-50 border border-gray-200 rounded-md min-h-[40px]">
                            {formData.title || '-'}
                        </div>
                                        {/if}
                                </div>
                                        <div>
                    <label for="jobEmployer" class="block text-sm font-medium text-gray-700 mb-1">Employer</label>
                    {#if editMode}
                        <input
                            type="text"
                            id="jobEmployer"
                ...
                {#if editMode}
                    ...
                    ></textarea>
                            {:else}
                    <div class="p-2 bg-gray-50 border border-gray-200 rounded-md min-h-[90px] whitespace-pre-wrap">
            ...
            <!-- Apply Link -->
                                                        <div>
                <label for="jobApplyLink" class="block text-sm font-medium text-gray-700 mb-1">Apply Link</label>
                {#if editMode}
                    <input
                        type="url" 
                        id="jobApplyLink"
                    ...
                    <div class="p-2 bg-gray-50 border border-gray-200 rounded-md min-h-[40px]">
                        {#if formData.apply_link}
                            <a href={formData.apply_link} target="_blank" class="text-blue-600 hover:underline font-medium">Apply link</a>
                        {:else}
                            -
                        {/if}
        </div>
    {/if}
            ...
            <!-- Seek Job ID -->
             <div>
                <label for="jobSeekJobId" class="block text-sm font-medium text-gray-700 mb-1">Seek Job ID</label>
                {#if editMode}
                    <input
                        type="text"
                        id="jobSeekJobId"
        ...
        {/if}
            ...
            <!-- Formatted Description -->
                <div>
                <label for="jobFormattedDescription" class="block text-sm font-medium text-gray-700 mb-1">Formatted Description</label>
                {#if editMode}
                    <textarea
                    ...
                    ></textarea>
                {:else}
                    <div class="p-2 bg-gray-50 border border-gray-200 rounded-md min-h-[120px] whitespace-pre-wrap">
            ...
            <div class="grid grid-cols-3 gap-4 pt-2 border-t mt-4">
                {#each [
                    { key: 'data_analyst_score', label: 'Data Analyst' },
                    { key: 'data_engineer_score', label: 'Data Engineer' },
                    { key: 'data_scientist_score', label: 'Data Scientist' },
                    { key: 'software_engineer_score', label: 'Software Engineer' },
                    { key: 'ai_engineer_score', label: 'AI Engineer' },
                    { key: 'product_owner_score', label: 'Product Owner' }
                ] as score}
                <div>
                        <label for={`job${score.key}`} class="block text-sm font-medium text-gray-700 mb-1">{score.label} Score</label>
                        {#if editMode}
                            <input
                    type="number"
                                id={`job${score.key}`}
