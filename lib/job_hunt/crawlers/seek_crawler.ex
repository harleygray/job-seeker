defmodule JobHunt.Crawlers.SeekCrawler do
  @moduledoc """
  Controls the Node.js-based Seek crawler through a port.
  Specifically designed to crawl Seek job listings and save them to the database.
  """
  ...
  # Use our Seek-specific crawler script
  @crawler_script "lib/job_hunt/crawlers/seek_crawler.js"

  # Store crawler process information
  @crawler_info :seek_crawler_info
  ...
  def start_crawler(urls, opts \\ [], callback_fn \\ nil) do
    ...
    case is_crawler_running() do
      true ->
        Logger.warning("Seek crawler is already running, ignoring duplicate start request", [])
  ...
  defp start_new_crawler(urls, opts, callback_fn) do
    ...
    case install_dependencies() do
      :ok ->
        ...
        opts = if Application.get_env(:job_hunt, :environment) == :prod do
  ...
  defp start_node_process(urls, opts, callback_fn) do
    Logger.info("Starting Node.js process for Seek crawler")
    ...
    case System.find_executable("node") do
      ...
      node_path ->
        ...
        case File.exists?(script_path) do
          true ->
            Logger.info("Found Seek crawler script at: #{script_path}")
            ...
            # Build configuration for Node.js with Seek-specific options
            config = %{
              urls: urls,
              headless: Keyword.get(opts, :headless, true),
              maxRequestsPerCrawl: Keyword.get(opts, :maxRequestsPerCrawl, 50),
              maxConcurrency: Keyword.get(opts, :maxConcurrency, 1), # Keep low for Seek
              navigationTimeoutSecs: Keyword.get(opts, :navigationTimeoutSecs, 180), # Increased to 3 minutes
              minDelayBetweenRequests: Keyword.get(opts, :minDelayBetweenRequests, 8000), # Increased delays
              maxDelayBetweenRequests: Keyword.get(opts, :maxDelayBetweenRequests, 15000),
              # Add pagination configuration options
              enablePagination: Keyword.get(opts, :enable_pagination, true),
              maxPages: Keyword.get(opts, :max_pages, 5)
            }

            # Log pagination settings if enabled
            if Keyword.get(opts, :enable_pagination, true) do
          ...
          false ->
            Logger.error("Seek crawler script not found at: #{script_path}")
  ...
  defp process_port_with_callback(port, callback_fn) do
    ...
    spawn(fn ->
      ...
      # Set a watchdog timeout of 10 minutes for Seek (longer due to anti-bot measures)
      receive do
        ...
        after
          7_200_000 ->  # 120 minute timeout for Seek (increased due to anti-detection delays)
    ...
    receive do
      {^port, {:data, data}} ->
        ...
        message_processed = if String.contains?(data, "ELIXIR_JSON_MESSAGE") do
          ...
          # Forward to the SeekCrawlerServer to process the structured message
          case JobHunt.Crawlers.SeekCrawlerServer.process_message(data) do
            {:ok, :page_complete} ->
        ...
        if page_complete_confirmation && !message_processed do
          ...
          Logger.info("Seek crawler completion detected via confirmation, cleaning up")
          cleanup_port(port)
          mark_crawler_stopped()
        ...
        if complete_via_marker && !message_processed do
          ...
          # This was a completion message, so we can clean up
          Logger.info("Seek crawler completion detected via markers, calling callback and cleaning up")
          cleanup_port(port)
          mark_crawler_stopped()
        ...
        if complete do
          Logger.info("Seek crawler run completed, calling callback")
      ...
      case Regex.scan(~r/(\{.*?\}|\[.*?\])/s, string) do
        ...
        matches ->
          ...
          Enum.find_value(matches, {:error, "No valid JSON found"}, fn [potential_json | _] ->
            case JSON.decode(potential_json) do
              {:ok, decoded} -> {:ok, decoded}
              _ -> nil
            end
          end)
  ...
  defp process_port_output(port, collected_data) do
        ...
        cond do
          ...
          true ->
            ...
            case JSON.decode(data) do
              ...
              {:ok, %{"type" => "complete"}} ->
                Logger.info("Seek crawler completed successfully")
                # Mark crawler as stopped
                mark_crawler_stopped()
                {:ok, port, Enum.reverse(collected_data)}

              {:ok, %{"type" => "error", "data" => error}} ->
                Logger.error("Seek crawler failed: #{inspect(error)}")
                # Mark crawler as stopped
                mark_crawler_stopped()
                {:error, error}
    ...
    after
        300_000 ->  # 300 second timeout (5 minutes) for Seek
  ...
  defp install_dependencies do
    Logger.info("Checking Node.js dependencies for Seek crawler")
    ...
    unless File.exists?(package_json) do
      ...
      File.write!(package_json, JSON.encode!(%{
        "name" => "job-hunt-seek-crawler",
        "version" => "1.0.0",
        "dependencies" => %{
          "crawlee" => "^3.13.0",
          "playwright" => "^1.51.0"
  ...
  @doc """
  Convenience function to start crawling Seek with a processor function that saves jobs to the database.
  """
  def crawl_and_save_jobs(urls, opts \\ []) do
    # Create a processor function that saves jobs to the database
    processor_fn = fn results_collection ->
      save_jobs_to_database(results_collection)
    end

    # Start crawling with the processor function
    JobHunt.Crawlers.SeekCrawlerServer.start_crawling(urls, opts, processor_fn)
  end

  @doc """
  Save jobs from the crawler results to the database.
  """
  def save_jobs_to_database(results_collection) do
    results = results_collection[:results] || []
    metadata = results_collection[:metadata] || %{}

    Logger.info("Saving #{length(results)} Seek jobs to database")
    ...
    # Process each job
    final_stats = Enum.reduce(results, stats, fn job_data, acc_stats ->
      try do
        ...
        job_attrs = %{
          ...
          salary: job_data["salary"] || "Not specified",
          apply_link: job_data["apply_link"] || "",
          seek_job_id: job_data["seek_job_id"] || "",
          description: job_data["description"] || ""
        }

        # Skip if we don't have essential data
        if job_attrs.title == "" || job_attrs.employer == "" || job_attrs.seek_job_id == "" do
          Logger.warning("Skipping job with missing essential data: #{inspect(job_attrs)}")
          Map.update(acc_stats, :skipped, 0, &(&1 + 1))
        else
          # Try to find existing job by seek_job_id
          case Repo.get_by(Job, seek_job_id: job_attrs.seek_job_id) do
            nil ->
              # Create new job
              case Context.create_job(job_attrs) do
                {:ok, _job} ->
                  Logger.debug("Created new job: #{job_attrs.title} at #{job_attrs.employer}")
                  Map.update(acc_stats, :created, 0, &(&1 + 1))
                {:error, changeset} ->
                  Logger.error("Failed to create job: #{inspect(changeset.errors)}")
                  Map.update(acc_stats, :errors, 0, &(&1 + 1))
              end

            existing_job ->
              # Update existing job
              case Context.update_job(existing_job, job_attrs) do
                {:ok, _job} ->
                  Logger.debug("Updated existing job: #{job_attrs.title} at #{job_attrs.employer}")
                  Map.update(acc_stats, :updated, 0, &(&1 + 1))
                {:error, changeset} ->
                  Logger.error("Failed to update job: #{inspect(changeset.errors)}")
                  Map.update(acc_stats, :errors, 0, &(&1 + 1))
              end
          end
        end
        ...
        e ->
          Logger.error("Exception while processing job: #{Exception.message(e)}")
          Logger.error("Job data: #{inspect(job_data)}")
          Map.update(acc_stats, :errors, 0, &(&1 + 1))
      end
    end)

    Logger.info("Job processing complete. Stats: #{inspect(final_stats)}")

    # Return the results collection with added stats
    Map.put(results_collection, :database_stats, final_stats)
  end
end
